...
double liouvillian::calc_single_site_mag(ITensor& rho, int i)
{
    // get indices
    IndexSet s = rho.inds();
    int N = s.r();
...
//     std::cout << " looking at coefficient ijklmn: " << i << j  << k  << l << m << n << std::endl;
//     std::cout << "site_mag: " << site_mag << std::endl;
    
    return (-1.0)*real(site_mag); 
}
