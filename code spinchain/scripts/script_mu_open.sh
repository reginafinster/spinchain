#!/bin/bash
CURRENTPATH="$PWD" 
#CURRENTPATH='/home/rfinster/Masterarbeit/Code/code_aktueller_Stand_neu/open_chain_transient/'
...
RUNPATH="$CURRENTPATH/mu_$mu/h_$disorder/realization_$realization"
mkdir -p $RUNPATH
param=`echo "scale=3; $mu/100"|bc -l`
param2=`echo "scale=3; $disorder/100"|bc -l`
sed "s/#mu/$param/g" < parameters > parameters_temp.cfg
sed "s/#disorder/$param2/g" < parameters_temp.cfg > open_chain_random_mu${param}.cfg
rm parameters_temp.cfg
mv open_chain_random_mu${param}.cfg $RUNPATH
cp heisenberg_liouville_space $RUNPATH
cd $RUNPATH
echo "realization $realization mu $mu disorder $disorder"
# ./heisenberg_liouville_space open_chain_random_mu${param}.cfg
qsub -mem 2 -args open_chain_random_mu${param}.cfg heisenberg_liouville_space
sleep 1
cd $CURRENTPATH
